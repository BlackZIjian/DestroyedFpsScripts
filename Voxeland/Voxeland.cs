...
namespace Voxeland5
{
	...
	#endif
	{
		...
		public static readonly int version = 531;   ///<For serialization purpose', to load older versions as a special cases.
		...
		public void Update()
		{
			...
			{
				UnityEditor.EditorApplication.update -= ThreadWorker.Refresh;
				return;
			}
#endif
			...
			{
				camPoses = Extensions.GetCamPoses(genAroundMainCam: genAroundMainCam,
					genAroundTag: genAroundObjsTag ? genAroundTag : null, camPoses: camPoses);
				...
				camPoses = new Vector3[]
					{new Vector3(terrainSize / 2, 0, terrainSize / 2)}; //static cam pos needed only to set priority
			...
			{
				areaDeployRects = new CoordRect[camPoses.Length];
				areaRemoveRects = new CoordRect[camPoses.Length]; //TODO: wtf, why do I create them twice?
				chunkDeployRects = new CoordRect[camPoses.Length];
				chunkRemoveRects = new CoordRect[camPoses.Length];
			...
			else
			{
				...
				areaDeployRects = new CoordRect[]
					{CoordRect.PickIntersectingCellsByPos(terrainSizeRect.Extend(margin + 1), cellSize: data.areaSize)};
				chunkDeployRects = new CoordRect[] {CoordRect.PickIntersectingCellsByPos(terrainSizeRect, cellSize: chunkSize)};
				areaRemoveRects = new CoordRect[] {areaDeployRects[0]};
				chunkRemoveRects = new CoordRect[] {chunkDeployRects[0]};
			}
			...
			if (!ArrayTools.EqualsVector3(prevCamPoses, camPoses, delta: 1f) || chunksChange || areasChange)
			{
				...
				foreach (Data.Area area in data.areas.All())
					area.generateWorker.priority =
						1f / camPoses.DistToRectCenter(area.rect.offset.x, area.rect.offset.z, area.rect.size.x) -
						0.00003f; //note that area priority is lower than the chunk one - chunk will wait for area anyways
				...
				if (horizon != null)
				{
					...
					if (camPosesCenter.DistAxisAligned(horizonPosition) >= horizon.repositionThreshold * chunkSize ||
					    horizon.heightmap == null)
		...
		#region Events and actions
			...
			public void Refresh (bool force=false) //called on generator change
			{
