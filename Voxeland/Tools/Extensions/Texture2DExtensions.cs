...
namespace Voxeland5
{
	public static class TextureExtensions
	{
			...
			{
				Texture2D readTex = new Texture2D(tex.width, tex.height, tex.format, mipmap:true, linear:tex.IsLinear());
			...
			/// Texture should be readable
			{
				...
				Texture2D resultTex = new Texture2D(tex.width, tex.height, TextureFormat.ARGB32, mipmap:mipmapCount!=1, linear:tex.IsLinear());
			...
			{
				Texture2D newTex = new Texture2D(newWidth, newHeight, TextureFormat.ARGB32, mipmap:true, linear:tex.IsLinear()); //has to be ARGB32 to set pixels
